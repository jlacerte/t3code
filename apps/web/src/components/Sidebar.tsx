...
function SidebarBrand() {
  // Le badge de stage (Dev/Alpha/Nightly) reste calculé pour usage interne
  // (data-app-stage) mais n'est plus affiché visuellement dans la sidebar.
  const stageLabel = useSidebarStageLabel();

  return (
    <Link
      aria-label="Aller aux fils"
      data-app-stage={stageLabel}
      className="sidebar-brand ml-[var(--workspace-titlebar-content-left)] w-fit min-w-0 shrink-0 flex-col items-start justify-center overflow-hidden rounded-md text-foreground outline-hidden ring-ring focus-visible:ring-2"
      to="/"
    >
      <span className="truncate text-sm leading-tight font-medium tracking-tight text-foreground">
        {APP_BASE_NAME}
      </span>
      <span className="truncate text-[10px] leading-tight font-medium text-muted-foreground/70">
        v{APP_VERSION}
      </span>
    </Link>
  );
